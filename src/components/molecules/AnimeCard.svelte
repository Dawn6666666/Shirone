...
>
	...
	<div class="anime-card__body">
		...
		<span class="anime-card__title">{anime.title}</span>
...
.anime-card
	...
	/* 悬停播放层：深色 scrim + 圆形播放钮（仅 link 卡片），hover 淡入 */
	&__play
		position: absolute
		inset: 0
		display: flex
		...
		background: unquote("color-mix(in srgb, #000 35%, transparent)")
		opacity: 0
	...
	/* 评分 scrim pill：图片上需要固定深底白字保证对比度 */
	&__rating
		position: absolute
		top: 0.5rem
		right: 0.5rem
		...
		background: unquote("color-mix(in srgb, #000 55%, transparent)")
		color: #fff
	...
	&__status
		...
		border-radius: var(--shape-corner-full)
		background: unquote("color-mix(in oklab, var(--anime-status-color) 14%, transparent)")
