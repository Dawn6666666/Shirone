...
test("resolves only manifest-declared Markdown stylesheet packs", async () => {
	...
	assert.deepEqual(
		...
		["acfun", "artplayer", "bilibili", "image-grids", "trees", "youtube"],
	);
	assert.match(assets[0].css, /\.m3-acfun/);
	assert.match(assets[1].css, /\.m3-artplayer/);
	assert.match(assets[2].css, /\.m3-bilibili/);
	assert.match(assets[3].css, /\.image-grid/);
	assert.match(assets[4].css, /\.m3-file-tree/);
	assert.match(assets[5].css, /\.m3-youtube/);
});
