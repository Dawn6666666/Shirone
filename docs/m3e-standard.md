...
| ProgressIndicator | `ProgressIndicator.svelte` | 进度指示（官方 ProgressIndicator.kt）：`variant`（linear 4dp 高轨道 / circular 40dp 圆环 4dp 厚）、`progress`（0-1 定值；省略/undefined = indeterminate 扫过动画）、`label`；active=primary、track=surface-container-highest；Linear 定值宽度过渡 250ms；Circular 用 SVG stroke-dasharray（周长 2πr） | primary / surface-container-highest |
