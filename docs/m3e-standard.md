...
| ButtonGroup | `ButtonGroup.svelte` | M3E 按钮组（官方 ButtonGroup.kt 数据驱动版）：`items: {value,label,icon?}[]`、`value`（$bindable 单选）/ `checkedValues`（$bindable 多选，`multiple` 时）、`variant`（standard 12px 间距 / connected 2px 间距）、`disabled`、`onchange`；connected：**首/尾项外侧全圆 + 内侧 8px、中间项 4dp，按压内角 8→4px，选中项变全圆 pill**（官方 ConnectedButtonGroupTokens）；**按压宽度膨胀 15%**（官方 pressed-item-width-multiplier，flex-grow 简化版）；溢出指示器 / animateWidth 动态宽度未实现（TODO） | secondary-container / secondary、`--m3e-elevation-1` |
